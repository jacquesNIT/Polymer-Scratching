...
def post_process(job_name, file_name, cfg):
    ...
    if _surf_key not in _surfaces.keys():
        try:
            _size_mb = os.path.getsize(odb_path) / (1024.0 * 1024.0)
        except OSError:
            _size_mb = float("nan")
        _present = sorted(_surfaces.keys())
        _steps = sorted(odb.steps.keys())
        odb.close()
        raise KeyError(
            "Surface %r absent from %s (%.1f MB). Surfaces present: %s. Steps: %s. An ODB of a few MB or less means the analysis died before writing model data -- read the .dat file, not this message."
