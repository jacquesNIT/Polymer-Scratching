...
JOBS = [
    ...
    ("depth", "glassy_pmma", {"tag": "Calib_ta1", "indenter": "rockwell", "ALE": True, "distortion": False, "scratch_time": 0.01, "unload_time": 0.005,"recovery_time": 0.005}),
...
# [PATCH:io-robustness] decoupage par defaut de TOUTE etude non listee ci-dessus.
# Une etude 'depth' a 3 cas tournait jusqu'ici dans un seul job SLURM,
# les 3 cas en serie dans le meme repertoire. Poser 'chunks' dans les
# opts d'une entree JOBS (ex. {"tag": "X", "chunks": 3}) donne un job
# SLURM et un repertoire par tranche.
_DEFAULT_CHUNKS = {"material": SWEEP_JOBS, "design": DESIGN_JOBS}
...
# [PATCH:io-robustness] begin
def _resolve_chunks(study, chunks_opt, entry):
    """Nombre de tranches SLURM : 'chunks' explicite, sinon defaut."""
    if chunks_opt is None:
        return _DEFAULT_CHUNKS.get(study, 1)
    try:
        n = int(chunks_opt)
    except (TypeError, ValueError):
        raise SystemExit("Bad 'chunks' value %r in JOBS entry %r: "
                         "expected a positive integer." % (chunks_opt, entry))
    if n < 1:
        raise SystemExit("Bad 'chunks' value %r in JOBS entry %r: "
                         "need chunks >= 1." % (chunks_opt, entry))
    return n
...
def expand_jobs():
    ...
    for entry in JOBS:
        ...
        # [PATCH:io-robustness] 'chunks' est consomme ici, pas un override cfg.
        opts = dict(entry[2]) if len(entry) == 3 else {}
        _chunks_opt = opts.pop("chunks", None)
        ...
        n_chunks = _resolve_chunks(study, _chunks_opt, entry)
        if n_chunks > 1:
